...
| `Update(ctx)` | `brew update` | 30m (or the caller's, whichever is nearer) | State-changing; runs under the caller's context so the update run's cancellation stops it |
...
`internal/views/pageview` is one of the twelve puregotk-free leaf packages under
...
`internal/views/actionstate` is one of the twelve puregotk-free leaf packages
...
`internal/views/badgestate` is one of the twelve puregotk-free leaf packages
...
`internal/views/bundleview` is one of the twelve puregotk-free leaf packages
...
`internal/views/progresslog` is one of the twelve puregotk-free leaf packages under `internal/views/`. It bounds what the two OS staging handlers render from a streamed run. Like `rowset` it imports nothing outside the standard library (`sync`, `time`) and names no widget type.
...
| `Update(ctx, appID, user)` | `flatpak update -y [--user\|--system] [<appID>]` | 30m (or the caller's, whichever is nearer) | State-changing; empty appID updates all; runs under the caller's context so the update run's cancellation stops it |

...
- **A visible retryable control must reset its action gate.**
  ...
  rollback, driver switching, Powerwash, and Factory Reset restore their
  buttons after a run, so they must call `Reset` when the run ends, including
  failure and dry-run paths. `internal/views/actionstate`'s wiring test guards
  every one of these controls. Both the dedicated bootc stage action and
  Update All's OS phase refresh the badge and changelog's Compare references
  from the new status, rather than leaving Compare disabled until restart.

...
func TestUpdateAllDryRunDoesNotAnnounceUpdates(t *testing.T) {
	...
	hide := strings.Index(clickedBody, "uh.updateAllRestart.SetVisible(false)")
	if guard < 0 || hide <= guard {
		t.Error("Update All must guard restart-row hiding so previews preserve known state")
	}

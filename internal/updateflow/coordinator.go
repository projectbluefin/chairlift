...
func (c *Coordinator) UpdateAll(
		...
		if sourceIndexValue >= 0 {
			...
			if err != nil {
				if errors.Is(err, context.Canceled) || errors.Is(ctx.Err(), context.Canceled) {
					...
					state.Progress = ""
					state.Generation = generation
					canceledState := derive(state)
					stateMu.Unlock()
					c.commit(generation, canceledState, publish)
					break
				}
				source.ApplyErr = err
				source.Completed = false
	...
	if ctx.Err() != nil {
		for i := range state.Sources {
			state.Sources[i].Updating = false
		}
	}
	state.Generation = generation
	state = derive(state)
	...
	if ctx.Err() == nil && (targetCount > 0 || retryMaintenance) && !hadFailure && !hadPreview &&
